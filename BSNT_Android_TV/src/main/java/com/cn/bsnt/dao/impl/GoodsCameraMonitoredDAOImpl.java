...
public class GoodsCameraMonitoredDAOImpl implements BaseDAO<GoodsCameraMonitored>{
	...
	//根据编号得到所有商品信息
	public List<GoodsCameraMonitored> selectAllById(int id){
		List<GoodsCameraMonitored> cameraMonitoredList=new ArrayList<GoodsCameraMonitored>();
		conn=ConnectionManager.getConnection();
		try {
			ps=conn.prepareStatement("select * from goods_camera_monitored where id=?");
			ps.setInt(1, id);
			rs=ps.executeQuery();
			while(rs.next())
			{
				...
				String booleanNeedDriver = rs.getString("BOOLEAN_NEED_DRIVER");
				String pixel = rs.getString("PIXEL");
				String photosensitiveOriginalType = rs
						.getString("PHOTOSENSITIVE_ORIGINAL_TYPE");
				String maxResolution = rs.getString("MAX_RESOLUTION");
				String condition = rs.getString("CONDITION");
				String additionFunction = rs.getString("ADDITION_FUNCTION");
				String networkConnectWay = rs.getString("NETWORK_CONNECT_WAY");
				...
				String shotSizeStandard = rs.getString("SHOT_SIZE_STANDARD");
				String memberCapacity = rs.getString("MEMBER_CAPACITY");
				String surverillanceCameraWay = rs
						.getString("SURVERILLANCE_CAMERA_WAY");
				GoodsCameraMonitored gcm = new GoodsCameraMonitored(id, name,
						price, type, color, size, brand, count, monthSales,
						totalSales, desc, firstCost, keepYears, definition,
						focus, focal, senseArea, sameCityService,
						appearanceMolding, supportOs, booleanNeedDriver, pixel,
						photosensitiveOriginalType, maxResolution, condition,
						additionFunction, networkConnectWay, storageType,
						effectiveDistance, smartType, shotSizeStandard,
						memberCapacity, surverillanceCameraWay);
				cameraMonitoredList.add(gcm);
				}
			} catch (SQLException e) {
