...
public class GoodsCameraDAOImpl implements BaseDAO<GoodsCamera>{
	...
	//得到所有的商品信息
	public List<GoodsCamera> selectAllById(int id) {
		List<GoodsCamera> CameraList = new ArrayList<GoodsCamera>();
		conn = ConnectionManager.getConnection();
		try {
			ps = conn.prepareStatement("select * from goods_camera where id=?");
			ps.setInt(1, id);
			rs = ps.executeQuery();
			while (rs.next()) {
				String name = rs.getString("NAME");
				...
				String slrFormat = rs.getString("SLR_FORMAT");
				String focalLength = rs.getString("FOCAL_LENGTH");
				String booleanFuselageAntiShank = rs
						.getString("BOOLEAN_FUSELAGE_ANTI_SHAKE");
				String sensorDesc = rs.getString("SENSOR_DESC");
				String sensorSize = rs.getString("SENSOR_SIZE");
				String viewfinderType = rs.getString("VIEWFINDER_TYPE");
				String storageType = rs.getString("STORAGE_TYPE");
				String booleanFushlageMotor = rs
						.getString("BOOLEAN_FUSHLAGE_MOTOR");
				String ccdType = rs.getString("CCD_TYPE");
				String booleanFushlageDedustring = rs
						.getString("BOOLEAN_FUSHLAGE_DEDUSTRING");
				String booleanFushlageCircumscribedFlashlight = rs
						.getString("BOOLEAN_FUSHLAGE_CIRCUMSCRIBED_FLASHLIGHT");
				String batteryWay = rs.getString("BATTERY_WAY");
				String slrLevel = rs.getString("SLR_LEVEL");
				String screenSize = rs.getString("SCREEN_SIZE");
				String lcdSize = rs.getString("LCD_SIZE");
				...
				String continuousCapture = rs.getString("CONTINUOUS_CAPTURE");
				String autodyne = rs.getString("AUTODYNE");
				String booleanFushlageFlashlight = rs
						.getString("BOOLEAN_FUSHLAGE_FLASHLIGHT");
				String flashlightCharacter = rs
						.getString("FLASHLIGHT_CHARACTER");
				String slrPriceSection = rs.getString("SLR_PRICE_SECTION");
				String dataInterface = rs.getString("DATA_INTERFACE");
				String effectivePixel = rs.getString("EFFECTIVE_PIXEL");
				String applySence = rs.getString("APPLY_SENCE");
				...
				String closeUpDistance = rs.getString("CLOSE_UP_DISTANCE");
				String booleanFlashlight = rs.getString("BOOLEAN_FLASHLIGHT");
				String photographicPaperSize = rs
						.getString("PHOTOGRAPHIC_PAPER_SIZE");
				String imageProcessor = rs.getString("IMAGE_PROCESSOR");
				String shortShoot = rs.getString("SHORT_SHOOT");
				String opticalZoomMultiples = rs
						.getString("OPTICAL_ZOOM_MULTIPLES");
				GoodsCamera gc = new GoodsCamera(id, name, price, type, color,
						size, brand, count, monthSales, totalSales, desc,
						firstCost, digitalPixel, weight, slrFormat,
						focalLength, booleanFuselageAntiShank, sensorDesc,
						sensorSize, viewfinderType, storageType,
						booleanFushlageMotor, ccdType,
						booleanFushlageDedustring,
						booleanFushlageCircumscribedFlashlight, batteryWay,
						slrLevel, screenSize, lcdSize, maxResolution, lcdPixel,
						focusingWay, exposureModel, exposureSompenstae,
						photometryWay, isoSensitivity, whiteBalanceModel,
						shutter, continuousCapture, autodyne,
						booleanFushlageFlashlight, flashlightCharacter,
						slrPriceSection, dataInterface, effectivePixel,
						applySence, additionFunction, screenType, shotType,
						apertureRange, shootDistance, closeUp, pictureSize,
						series, combaType, booleanCloseUp, closeUpDistance,
						booleanFlashlight, photographicPaperSize,
						imageProcessor, shortShoot, opticalZoomMultiples);
				CameraList.add(gc);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
